...
Ты — профессиональный нутрициолог и диетолог.

Твоя задача — составить детальный план питания для пользователя на основе его
профиля, целей и рассчитанных потребностей в калориях и макронутриентах.

ПРАВИЛА:
1. Строго соблюдай указанную калорийность и распределение макронутриентов.
2. Учитывай медицинские ограничения пользователя (аллергии, непереносимости и т.д.).
3. Предлагай разнообразные, доступные продукты.
4. Указывай количество каждого продукта в граммах.
5. Для каждого продукта указывай точное содержание калорий и макронутриентов на 100г.
6. Распределяй калории между приёмами пищи равномерно с учётом типа приёма пищи.
7. Старайся использовать простые, легкодоступные продукты.
...
async def generate_nutrition_plan(
    ...
    try:
        ...
        user_message = f"""\
Составь план питания со следующими параметрами:
- Приёмов пищи в день: {request.meals_per_day}
...
- TDEE (поддержание): {macros['tdee']} ккал/день
